...
window.onload = () => {
  const tbody = document.querySelector('#kategoriTablo tbody');
  ...
  fetch('/kategoriListesi')
    ...
    .then(list => {
      ...
      const sel = document.getElementById('e_kategori_adi');
      list.forEach(u => {
        const opt = document.createElement('option');
        opt.value = u.e_kategori_adi;
        opt.textContent = `${u.e_ad} ${u.e_soyad} (${u.e_kategori_adi})`;
        sel.appendChild(opt);
      });
    })
    ...
    const badgeSinifi = durumDegeri.charAt(0).toUpperCase() + durumDegeri.slice(1);
      ...
      tr.innerHTML = `
        ...
        <td class="text-center flex w-100 justify-content-center gap-2">
          ...
          <button class="bin-button w-50" onclick="kullaniciSil('${u.e_id}')">
            <!-- SVG sil ikonu -->
  ...
  //     u.e_kategori_adi.toLowerCase().includes(q) ||
  ...
  window.kullaniciDuzenlemeModunaGec = (e_id) => {
    ...
    if (secilenKayit) {
      document.getElementById('e_kategori_adi').value = secilenKayit.e_kategori_adi || '';
  ...
  window.kullaniciOlusturModal = () => {
    ...
    document.getElementById('e_kategori_adi').value = '';
...
window.islemiKaydet = () => {
  const e_kategori_adi = document.getElementById('e_kategori_adi').value;
