...
window.onload = () => {
        ...
        <td>${gorev.e_gorevli_kullanici}</td>
            ...
            <td class="text-center w-10">
              <div class="flex flex-row gap-2 w-100">
                <button class="Btn" onclick="gorevTanimlariModal('${gorev.e_id}')">
                ...
                <button class="bin-button" onclick="sil('${gorev.e_id}', '${gorev.e_onaylayan_kullanici}', '${gorev.e_gorevli_kullanici}', '${gorev.e_gorev}', '${gorev.e_durum}')">
  ...
  //   const filt = users.filter(gorev =>
  //     gorev.e_onaylayan_kullanici.toLowerCase().includes(q) ||
  //     gorev.e_ad.toLowerCase().includes(q) ||
  //     gorev.e_soyad.toLowerCase().includes(q)
  //   );
  //   draw(filt);
  // });

  window.gorevTanimlariModal = (e_id) => {
    // Seçilen kullanıcıyı bul
    secilenKayit = users.find(gorev => gorev.e_id === e_id);
    ...
    if (secilenKayit) {
      ...
      document.getElementById('e_gorevli_kullanici').value = secilenKayit.e_ad || '';
      document.getElementById('e_gorev').value = secilenKayit.e_soyad || '';
  ...
  window.kullaniciDurumGuncelle = () => {
    ...
    const gidecek_gorevli = document.getElementById('e_onaylayan_kullanici').value;
    ...
    if (secilenKayit) {
      ...
      fetch('/gorevDuzenle', {
        ...
        body: JSON.stringify({
          ...
          e_gorevli_kullanici,
          e_gorev,
          e_onaylayan_kullanici: gidecek_gorevli
        })
      })
      .then(r => r.json())
      .then(res => {
        ...
        if(idx !== -1) {
          ...
          users[idx].e_gorevli_kullanici = e_gorevli_kullanici;
          users[idx].e_gorev = e_gorev;
          users[idx].e_onaylayan_kullanici = gidecek_gorevli;
        }
        draw(users);
        kapatModal();
      })
  ...
  window.sil = (e_id, e_onaylayan_kullanici, e_gorevli_kullanici, e_gorev, e_durum) => {
    if(confirm(`"${e_gorevli_kullanici} ${e_gorev}" isimli görevi silmek istediğinize emin misiniz?`)) {
      fetch('/gorevSil', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
