    ...
    .then(data => {
      ...
      const addDataToTable = (filteredData) => {
        ...
        filteredData.forEach(gorev => {
          ...
          durumDegeri === 'İptal Edildi'  ? 'bg-danger' :
          'bg-secondary';
          ...
          <td class="text-left">${gorev.e_kullanici_adi}</td>
...
// Kayıt silme
function sil( e_gorevli_kullanici, e_kullanici_adi, e_durum, e_onaylayan_kullanici, e_gorev ) {
  fetch('/gorevSil', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ e_gorevli_kullanici, e_kullanici_adi, e_durum, e_onaylayan_kullanici, e_gorev })
  })
    .then(res => res.json())
    .then(data => {
